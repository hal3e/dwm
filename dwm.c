...
drawbar(Monitor *m)
{
	int x, w, midtext, sw = 0, tx = 0;
	int boxs = drw->fonts->h / 9;
	int boxw = drw->fonts->h / 6 + 2;
	unsigned int i, cs, occ = 0, urg = 0;
  char *sub;
	Client *c;
  ...
  while(sub!=NULL){
    cs = ((unsigned int)*sub) - 48; /* substarct 48 to get number from asci 8*/
    if( cs >= 0 && cs < LENGTH(colors))
      drw_setscheme(drw, scheme[cs]);
    drw_text(drw, m->ww - sw + tx, 0, sw, bh, lrpad / 2, sub+1, 0);
    tx += TEXTW(sub + 1) - 10;
    sub = strtok(NULL,"|");
  }
	...
	x = 0;
	for (i = 0; i < LENGTH(tags); i++) {
		w = TEXTW((m->tagset[m->seltags] & 1 << i) ? tags[i] : "");
      ...
      drw_setscheme(drw, scheme[SchemeNord2]);
			drw_text(drw, x, 0, w, bh, lrpad / 2, tags[i], 0);
    }
	...
	if ((w = m->ww - sw - x) > bh) {
		if (m->sel) {
      midtext = (m->ww - TEXTW(m->sel->name)) / 2 - x;
			drw_setscheme(drw, scheme[SchemeNorm]);
			drw_text(drw, x, 0, w, bh, midtext, m->sel->name, 0);
			if (m->sel->isfloating){
        drw_setscheme(drw, scheme[SchemeNord2]);
				drw_rect(drw, x + boxs, (bh - boxw ) / 2, boxw, boxw, 1, 0);
      }
		} else {
      drw_setscheme(drw, scheme[SchemeNord2]);
...
focus(Client *c)
{
	...
	if (c) {
		...
		attachstack(c);
		grabbuttons(c, 1);
		XSetWindowBorder(dpy, c->win, scheme[SchemeNord10][ColBorder].pixel);
		setfocus(c);
	} else {
...
focusmaster(const Arg *arg)
{
  ...
  /* Select last master */
	...
	for (int m = 1; m < selmon->nmaster; master = nexttiled(master->next), m++);
